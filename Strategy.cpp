...
void BallFollow(Robot *robot, Environment *env);
...
extern "C" STRATEGY_API void Strategy ( Environment *env )
{
	...
	//0-vermelho(B1) 1-roxo escuro(B2) 2-Roxo (B3) 3-Verde(B4) 4-verde azul(B5)
	switch (env->gameState)
	{
		case 0:
			// default
			Velocity(&env->home[1], -100, -100);
		...
		case FREE_BALL:
			...
			BallFollow(&env->home[1], env);
			// Go inside goal
			//Position(&env->home[0], FRIGHTX + 10, GBOTY + 10);
			//Position(&env->home[3], FRIGHTX + 10, GBOTY + 7);
			
			Defend(&env->home[2], env, 29, 45);
			Goalkeeper(&env->home[0], env);
			
				// attack
			//Attack2(&env->home [4], env );
...
void Goalkeeper ( Robot *robot, Environment *env )
{
	...
	if ( Ay > Ty + 0.9 && Ay > 27 )
	{
		velocityLeft = -100;
		velocityRight = -100;
	//	Position(robot, env->goalBounds.right-10, env->currentBall.pos.y);
	...
	if ( Ay < 27 ) //Fora dos limites do gol na parte superior
	{
		velocityLeft = 100;
		velocityRight = 100;
	}

	if ( Ay > 43 ) //Fora dos limites do gol na parte inferior
	{
		velocityLeft = -100;
		velocityRight = -100;
	}
	...
	if ( Tr > 360.001 )
		Tr = Tr - 360;
	if ( Tr > 270.5 )
		velocityLeft = velocityLeft + fabs ( Tr - 270 ); //fabs valor absoluto em double
	else if ( Tr < 269.5 )
